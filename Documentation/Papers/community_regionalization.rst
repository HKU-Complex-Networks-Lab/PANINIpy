...
as specified in Eq. 9 of https://arxiv.org/pdf/2405.04911. The function :math:`C(B)` only depends on the number of clusters :math:`B` and other network-level constants, while :math:`g(r)` only depends on the r-th cluster and :math:`f(r,s)` only depends on the r-th and s-th clusters. This general functional form can accommodate most stochastic block model variants as well as modularity or infomap. 
...
Step 2: Load pickled data for the Baton Rouge CBSA (example datasets are available inside `PANINIpy/Example_dataset <https://github.com/HKU-Complex-Networks-Lab/PANINIpy/tree/main/Example_dataset>`_).
